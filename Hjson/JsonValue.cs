...
namespace Hjson
{
	...
	public abstract class JsonValue: IEnumerable
	{
		...
		public void Save(Stream stream, Stringify format = Stringify.Plain)
		{
			...
			this.Save(new StreamWriter(stream), format);
		}
		...
		/// <summary>Saves the JSON to a string.</summary>
		public override string ToString() => this.ToString(Stringify.Plain);

		/// <summary>Returns the contained primitive value.</summary>
		public object ToValue() => ((JsonPrimitive)this).Value;

		/// <summary>Wraps an unknown object into a JSON value (to be used with DSF).</summary>
		public static JsonValue FromObject(object value) => JsonPrimitive.FromObject(value);
